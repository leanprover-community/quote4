...
private def buildPatternTerm (discr : Term) (pat : Term) (body : Term) : MacroM Term := do
  ...
  | `(~q($inner)) =>
    -- Fast path for a whole-pattern `~q(...)`. Going through `floatQMatchPairs` /
    -- `let x := discr; _qq_match _ ← x` would obscure `discr`'s expression type at
    -- `_qq_match`'s elaboration call, leaving the universe metavariable in floated
    -- antiquotation bindings (`let a : Quoted _ := ...`) unresolved. Feeding `discr`
    -- directly preserves the type at the relevant elaboration phase.
    let (inner, lifts) ← (floatExprAntiquot 0 inner).run #[]
    liftsToTermLet lifts (← `(_qq_match $inner ← $discr | __alt in $body))
  | _ =>
    if hasQMatch pat then
      ...
      if isIrrefutablePattern pat' then
        -- Multi-q irrefutable case (e.g. `(~q(X), ~q(Y))`): the let destructures `discr`
        -- via the pattern, which propagates component types to the inner `_qq_match`s.
        `(let $pat':term := $discr; $body)
      else
