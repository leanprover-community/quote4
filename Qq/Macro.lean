...
def setUnquotedLevel (quotedMVar : Expr) (unquoted : Level) : MetaM Unit :=
...
constant unquoteStuck {α : Sort u} (a : α) : α := a

@[inline] constant betaRev' (e : Expr) (revArgs : List Expr) : Expr :=
  e.betaRev revArgs.toArray

partial def quoteExpr : Expr → TermElabM Expr
  | Expr.app (Expr.app (Expr.const ``unquoteStuck _ _) ty _) expr _ => expr
    ...
    | Expr.const c _ _, _ =>
      match c, nargs with
        ...
        | ``Expr.forallE, 4 =>
          withLocalDecl (← reduceEval (e.getArg! 0)) (← evalBinderInfoData (e.getArg! 3))
              (← unquoteExpr (e.getArg! 1)) fun x => do
            mkForallFVars #[x] (← unquoteExpr (e.getArg! 2))
        | ``Expr.letE, 5 =>
...
partial def unquoteExpr (e : Expr) : UnquoteM Expr := do
  try unquoteExprCore e catch _ => mkPostponedUnquoteExpr (← readThe MVarId) e
...
def unquotingLCtx (k : UnquoteM Expr) : TermElabM Expr := do
...
scoped elab "q(" t:term ")" : term <= expectedType => do
  ...
  let res ← mkApp2 (mkConst ``QQ.qq) quotedET <|<- mkPostponedQuoteExpr <|<-
    unquotingLCtx do
      ...
      elabTermEnsuringType t (← unquoteExpr quotedET)
...
scoped elab "Q(" t:term ")" : term => do
  ...
  let res ← mkApp (mkConst ``QQ) <|<- mkPostponedQuoteExpr <|<-
    unquotingLCtx do
      withPostponedTactics do
      elabTermEnsuringType t (mkSort (← mkFreshLevelMVar))
  let res ← instantiateMVars res
  -- dbg_trace "Q({t}) -> {res}"
  -- dbgPostponementState
  res
...
def orChange : (ps : List Q(Prop)) → Q(%(or1 ps) → %(or2 ps))
  ...
  | p::(ps1::ps2) =>
    ...
    q(fun h => Or.elim h Or.inl (fun h => Or.inr (ih h)))
